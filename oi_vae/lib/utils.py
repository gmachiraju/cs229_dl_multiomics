...
def KL_Normals(d0, d1):
    ...
    return torch.sum(-0.5 + (sigma0_sqr + torch.pow(d0.mu - d1.mu, 2)) / (2 * sigma1_sqr) + torch.log(d1.sigma) - torch.log(d0.sigma))
...
softplus = Bijection(
    ...
    forward_log_abs_det_jacobian=lambda x: torch.sum(
        x - torch.log(torch.exp(x) + 1)),
    inverse_log_abs_det_jacobian=lambda y: torch.sum(
        y - torch.log(torch.abs(torch.exp(y) - 1)))
)
