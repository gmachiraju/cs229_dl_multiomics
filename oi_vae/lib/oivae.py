...
class OIVAE(object):
    ...
    def step(self, X, prox_step_size, mc_samples):
        ...
        # works appropriately. # NOTE: ADDED 0.5 FACTOR
        z_kl = 0.01 * KL_Normals(q_z, self.prior_z.expand_as(q_z)) / batch_size
        ...
        loglik_term = (self.generative_model(
            z_sample).logprob(Xrep) / mc_samples / batch_size)
        ...
        # gradient clipping
        # pdb.set_trace()
        # torch.nn.utils.clip_grad_norm(self.optimizers[1].paratmers(), 10)
        # print(self.optimizers[1].parameters)
