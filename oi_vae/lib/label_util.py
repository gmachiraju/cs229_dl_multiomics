...
def load_labsn_hcpmmp1_7_labels(include_visual=False):
    ...
    ipc_labs = [
        l for l in hcp_mmp1_labels if 'Inferior Parietal Cortex' in l.name]
...
def load_labsn_hcpmmp1_av_rois_small():
    ...
    tpo_labs = [
        l for l in hcp_mmp1_labels if 'Temporo-Parieto-Occipital Junction' in l.name]
    labels.extend(tpo_labs)

    dpc_labs = [
        l for l in hcp_mmp1_labels if 'DorsoLateral Prefrontal Cortex' in l.name]
