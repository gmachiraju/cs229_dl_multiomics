...
def make_group_generator():
    ...
    log_sigma = Variable(
        torch.log(1e-2 * torch.ones(image_size)), requires_grad=True)
    return NormalNet(
        mu_net=torch.nn.Linear(group_input_dim, image_size),
        sigma_net=Lambda(
            lambda x, log_sigma: torch.exp(
...
def debug(count):
    ...
    for i in range(count):
        Xvar = Variable(X[[i]])
        ax[1, i].bar(range(dim_z), inference_net(
    ...
    for i in range(count):
        ...
        fX = generative_net(inference_net(Xvar).sample()
                            ).sample().view(image_size, image_size)
...
def debug_incoming_weights():
    ...
    for i in range(generative_net.Ws.size(0)):
        ...
        ax[i].imshow(torch.stack([m.data for _ in range(image_size)]
...
def debug_outgoing_weights():
    ...
    col_norms = torch.stack([
        torch.sqrt(
            torch.sum(torch.pow(generative_net.Ws[i].data.t(), 2), dim=0))
        for i in range(generative_net.Ws.size(0))
    ])
    ...
    for i in range(dim_z):
        ax[i].imshow(torch.stack([col_norms[:, i] for _ in range(
            image_size)]).squeeze(), vmin=-0.5, vmax=0.5, cmap=cmap)
...
optimizer = torch.optim.Adam([
    ...
    {'params': [gen.sigma_net.extra_args[0]
                for gen in generative_net.group_generators], 'lr': lr}
])
...
debug_z_by_group_matrix()
plt.savefig('figs/bars/bars_data_connectivity_matrix.pdf', format='pdf')
...
def save_img_and_reconstruction(ix):
    ...
    plt.savefig(
        'figs/bars/{}_reconstruction_full_sample.pdf'.format(ix), format='pdf')
